...
// ─────────────────────────────────────────────────────────
router.delete('/:id', checkPermission('can_add_loans'), destructiveActionLimiter, stepUpAuth, async (req, res) => {
    const { id } = req.params;
...
// ─────────────────────────────────────────────────────────
router.get('/:id', checkPermission('can_view_loans'), async (req, res) => {
    const { id } = req.params;
    const merchantId = req.merchantId;
...
router.get('/', async (req, res) => {
    try {
        ...
        const [countRes, dataRes] = await Promise.all([
            ...
            req.dbClient.query(
                `SELECT l.id, l.amount, l.receipt_number, l.receipt_image_url,
...
router.get('/:id', async (req, res) => {
    try {
        const r = await db.query(
            `SELECT l.*, c.full_name AS customer_name, c.national_id, c.mobile_number
             FROM loans l LEFT JOIN customers c ON l.customer_id = c.id
             WHERE l.id = $1 AND l.merchant_id = $2`,
            [req.params.id, req.merchantId]
        );
        if (!r.rows.length) return res.status(404).json({ error: 'Loan not found' });
        res.json(enrichLoan(r.rows[0]));
    } catch (err) {
    ...
    try {
        ...
        const r = await db.query(
            `INSERT INTO loans (
                ...
                najiz_case_number, najiz_case_amount, najiz_status, is_najiz_case
             )
...
router.patch('/:id', checkPermission('can_add_loans'), async (req, res) => {
    try {
        ...
        // Status Invariant: If not "Raised", force Najiz fields to NULL
        const finalStatus = status !== undefined ? status : undefined;
        ...
        // Logic force: Najiz fields only valid if status is 'Raised' or becoming 'Raised'
        // We check against the database status if 'status' is not in body? 
        // Safer: If status in body is NOT Raised, nullify. If status NOT in body, we might need to check DB.
        // For simplicity and safety during the update:
        const isRaised = (finalStatus === 'Raised');

        if (najiz_case_number !== undefined) {
            updates.push(`najiz_case_number = $${i++} `);
            params.push(isRaised ? najiz_case_number : null);
        }
        if (najiz_case_amount !== undefined) {
            updates.push(`najiz_case_amount = $${i++} `);
            params.push(isRaised ? najiz_case_amount : null);
        }
        if (najiz_status !== undefined) {
            updates.push(`najiz_status = $${i++} `);
            params.push(isRaised ? najiz_status : null);
        }
        if (najiz_collected_amount !== undefined) {
            updates.push(`najiz_collected_amount = $${i++} `);
            params.push(isRaised ? najiz_collected_amount : 0);
        }
        if (is_najiz_case !== undefined) {
            updates.push(`is_najiz_case = $${i++} `);
            params.push(is_najiz_case);
        } else if (status !== undefined) {
            // Auto-update is_najiz_case if status changes to/from Raised
            updates.push(`is_najiz_case = $${i++} `);
            params.push(isRaised);
        }
